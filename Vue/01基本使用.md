...
// Vue 会尽可能高效地渲染元素，通常会复用已有元素而不是从头开始渲染。这么做除了使 Vue 变得非常快之外，还有其它一    些好处。例如在验证码登录和密码登录的切换时，不带key时会复用input标签，当用户在验证码登录的输入框中输入手机号    后，再切换为密码登录，这时密码登录的用户名的值会是用户之前输入的手机号码，不需要这种效果子需要给输入框添加不一    样的key值即可。
...
<template>
	<div id="app">
		<label for="boy">
			<!-- 
                id 对应label的for， 
				name属性添加相同的值，表示一组，这样才可以单选
                v-model对应的值为value的值
            -->
			<input type="radio" id="boy" name="sex" value="1" v-model="sex"/>
			男
		</label>
		<label for="girl">
			<input type="radio" id="girl" name="sex" value="2" v-model="sex"/>
			女
		</label>
	</div>
</template>
...
```
 ...
 在这个地方，模板不再是简单的声明式逻辑。你必须看一段时间才能意识到，这里是想要显示变量 `message` 的翻转字符串。当你想要在模板中多包含此处的翻转字符串时，就会更加难以处理。 对于复杂逻辑，应当使用计算属性。
        ...
        watch: {
            ...
            list: {
                ...
                */
                ...
                不然改变数据内部应用数据类型中的值时不会触发该侦听。该选项比较消耗性能，慎用。
                */
                deep: true,
...
先创建一个`EventBus.js`，再这个文件中创建一个`vue`实例，并导出。
