...
侦听器和计算属性的功能类似，都有监测数据变化的的功能。

使用场景：当数据发生变化的同时需要进行异步操作（或开销比较大的操作） 就是用watch
...
- 发送post请求
  ...
  ```js
  axios.post('http://localhost:3000/brands', {name: '123', sex: '男'}, {
      headers: {
          token: '123'
      }
  }),
...
>   子组件向父组件通信用的是自定义事件
>
>   父组件定义一个自定义事件绑定在子组件上，子组件通过vm.$emit('事件名', 传递的值)触发事件



## 兄弟组件通信

>   兄弟组件通信是 子父组件通信(自定义事件)和 父子组件通信的结合 (props传值)
...
```vue
//如果Hello组件模板中没有 <slot></slot>标签，那么 ‘在组件标签间写入内容’ 这一行文字将不会被渲染
//Hello标签中间的文本的位置由 slot标签的位置决定
<div id="app">
        <Hello>在组件标签间写入内容</Hello>
    </div>
...
```vue
// Hello组件中插入的2个p标签， 即为 <slot></slot>内容，页面上显示为
Top
Bottom
Hello组件
Top
...
//具名插槽的使用
//给Hello组件标签中的p标签添加slot属性，值为一个名称，对应 <slot></slot>标签的name属性
<div id="app">
        <Hello>
            <p slot="top">Top</p>
...
- 组件：包含了 html、css和 js，而它的 js可以由各种模块组成,
