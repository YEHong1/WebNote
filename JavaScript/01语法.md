...
在 `JavaScript`中虽然可以通过字面量或者`Object`构造函数来创建单个对象，但是这些方法都有一个明显的缺点：使用同一个接口创建很对对象的时候，会产生大量的重复代码。
...
我们通过 `new`构建字调用构造函数的返回结果就是一个实例对象，构造函数中的 `this` 是指向实例对象的
...
// child可以访问父类的构造属性和原型方法，当是不能在实例化时给父构造函数传参
...
当一个对象被被赋值给另一个变量时，引用数加一。被赋值的变量修改为其它值后，该对象的引用数减一
